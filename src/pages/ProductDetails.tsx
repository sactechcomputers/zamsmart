  ...
  if (!product) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => navigate('/')} className="mt-4 text-orange-600 font-bold">Go back home</button>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          ...
          >
            <img 
              src={product.image_url || 'https://picsum.photos/seed/product/800/800'} 
              alt={product.name}
          ...
          <div className="grid grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="aspect-square rounded-xl overflow-hidden bg-white border border-slate-100 cursor-pointer hover:border-orange-500 transition-all">
                <img src={`https://picsum.photos/seed/p${i}/200/200`} alt="thumb" className="w-full h-full object-cover opacity-60 hover:opacity-100" referrerPolicy="no-referrer" />
              </div>
            ))}
          </div>
        </div>
